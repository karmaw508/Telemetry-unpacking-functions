...
'444': lambda data: [
	('BMS_State',				hex_to_uint16(get_byte(data, 0)))
],
'445': lambda data: [
...
'35A': lambda data: [
	('ECU_TPS_L'	 , hex_to_uint16(get_byte(data, 0) + get_byte(data, 1)) / 10.0 )
],
'35B': lambda data: [
...
'452': lambda data: [
	('FX_DIV_FZ_FL'	 , hex_to_int16(get_byte(data, 0) + get_byte(data, 1)) / 1000.0), 
	('FX_DIV_FZ_FR'	 , hex_to_int16(get_byte(data, 2) + get_byte(data, 3)) / 1000.0),
	('FX_DIV_FZ_RL'	 , hex_to_int16(get_byte(data, 4) + get_byte(data, 5)) / 1000.0),
	('FX_DIV_FZ_RR'	 , hex_to_int16(get_byte(data, 6) + get_byte(data, 7)) / 1000.0)
],
...
'285': lambda data: [
	...
	('AMK_FR_Torque_current',			hex_to_int16(get_byte(data,4)+get_byte(data,5)) * 0.0065),
	('AMK_FR_Magnetizing_current',		hex_to_int16(get_byte(data,6)+get_byte(data,7))	* 0.0065),
	('AMK_FR_Motor_torque',				hex_to_int16(get_byte(data,4)+get_byte(data,5)) * 0.0017)
],
#AMK Actual Values 2
'287': lambda data: [
